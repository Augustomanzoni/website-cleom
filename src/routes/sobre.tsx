...
function SobrePage() {
  return (
    <>
      <PageHero
        eyebrow="A CLEOM"
        title={<>ESPECIALIZAÇÃO EM<br />ENGENHARIA FRIGORÍFICA</>}
        subtitle="Projetando soluções para frigoríficos, indústrias alimentícias e centros de distribuição com excelência a mais de três anos."
      ...
      <section className="py-20 bg-background">
        <div className="container mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
          ...
          />
          <div>
            <p className="text-cyan-accent uppercase tracking-[0.3em] text-sm mb-3">Nossa missão</p>
            <h2 className="text-4xl text-navy-deep mb-6">Engenharia que move a indústria frigorífica</h2>
            <p className="text-muted-foreground mb-4">
              Fornecer soluções frigoríficas industriais que garantam a preservação dos produtos,
              reduzam custos operacionais e elevem a eficiência energética das nossas indústrias parceiras.
            </p>
            <p className="text-muted-foreground mb-4">
              Trabalhamos com equipamentos de última geração e mantemos parcerias com os principais
              fabricantes mundiais de sistemas de refrigeração industrial.
            </p>
            <p className="text-muted-foreground">
              Nossa equipe técnica especializada desenvolve projetos sob medida, desde câmaras frigoríficas
              até linhas completas de processamento.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-secondary">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto mb-12">
            <h2 className="text-4xl text-navy-deep">Diferenciais CLEOM</h2>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((f) => (
              <div key={f.title} className="bg-card p-6 rounded-2xl border border-border">
                ...
                <h3 className="text-xl text-navy-deep mb-2">{f.title}</h3>
