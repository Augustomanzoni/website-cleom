    ...
    <>
        ...
        <div className="container mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            <div key={s.l}>
              ...
              <div className="text-chrome/70 uppercase tracking-wider text-xs mt-2">{t(s.l)}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 bg-secondary">
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mb-12">
            <p className="text-cyan-accent uppercase tracking-[0.3em] text-sm mb-3">{t("Diferenciais")}</p>
            <h2 className="text-4xl md:text-5xl text-navy-deep">{t("Por que escolher a CLEOM")}</h2>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={b.title} className="bg-card p-6 rounded-2xl border border-border hover:border-cyan-accent transition-colors">
                ...
                <h3 className="text-xl mb-2 text-navy-deep">{t(b.title)}</h3>
      ...
      <section className="navy-gradient text-white py-20">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl md:text-5xl uppercase chrome-text mb-4">{t("PRONTO PARA ENCONTRAR SUA SOLUÇÃO?")}</h2>
          <p className="text-chrome/80 max-w-2xl mx-auto mb-8">
            {t("Nossa equipe de engenharia desenvolve projetos sob medida para sua operação.")}
          </p>
          <Link
            to="/contato"
          ...
          >
            {t("Fale com um especialista")} <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      </section>
    </>
